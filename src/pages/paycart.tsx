...
const Paycart = (props: Props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    try {
      const { _id: idOrder }: any = await create({ ...data, money: tong });
      cart.map((item: any) => {
        const product = {
          name: item.name,
          price: item.price,
            ...
            <div className="col-lg-6 col-md-8 col-sm-10 offset-lg-0 offset-md-2 offset-sm-1 pt-lg-0 pt-3">
              <div id="cart" className="bg-white rounded">
                ...
                {cart?.map((item: any) => (
                  <div
                    key={item._id}
                    className="d-flex jusitfy-content-between align-items-center pt-3 pb-2 border-bottom"
                  >
                    ...
                    <div className="ml-auto">
                      {" "}
                      <b className="h5">{currencyPrice(item.price)}</b>{" "}
                    </div>
                  </div>
                ))}
                <div className="my-3">
              ...
              <div className="row pt-lg-3 pt-2 buttons mb-sm-0 mb-2">
                <Link href={"/"}>
                  <div className="col-md-6">
                    <div className="btn text-uppercase">
              ...
              <div className="text-muted pt-3" id="mobile">
                {" "}
                <span className="fas fa-lock"></span> Your information is save{" "}
              </div>
            </div>
          </div>
        </form>
