...
const ListUsers = (props: Props) => {
  const { data: user, error, status } = useAuth()
          ...
          <tbody>
            {user?.map((item: any, index: number) => (
              <tr className="border-bottom" key={item._id}>
                ...
                <td>{item.name}</td>
                <td>{item.email}</td>
                <td><button className={`${item.status == true ? "btn btn-success" : "btn btn-danger"}`} onClick={() => status(item._id)}>{item.status == true ? "Activated" : "Disable"}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
